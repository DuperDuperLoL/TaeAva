...
struct Player {
    ...
    void readFromMemory(ConfigLoader* cl) {
        ...
        highlightId = mem::Read<int>(base + OFF_GLOW_HIGHLIGHT_ID + 0);
    ...
    void setCustomGlow(int health, bool isVisible, bool isSameTeam)
    {
        ...
        std::array<unsigned char, 4> highlightFunctionBits = {
            2,   // InsideFunction
            113, // OutlineFunction: HIGHLIGHT_OUTLINE_OBJECTIVE
            80,  // OutlineRadius: size * 255 / 8
            127   // (EntityVisible << 6) | State & 0x3F | (AfterPostProcess << 7)
        };
        ...
        if (isSameTeam) {
            ...
            settingIndex = 65;
            glowColorRGB = { 3, 0, 0 }; // knocked enemies // gray
        } else if (health >= 205) {
            settingIndex = 66;
            glowColorRGB = { 10, 0, 10 }; // red shield
        } else if (health >= 190) {
            settingIndex = 67;
            glowColorRGB = { 5, 0, 10 }; // purple shield
        } else if (health >= 170) {
            settingIndex = 68;
            glowColorRGB = { 0, 5, 10 }; // blue shield
        } else if (health >= 95) {
            settingIndex = 69;
